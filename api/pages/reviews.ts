...
const DEFAULT_METADATA = {
  pageUrl: 'https://hawlariza.com/reviews',
  pageTitle: 'Reviews | Heal in Paradise - Reader Testimonials',
  pageDescription: 'Read what readers are saying about Heal in Paradise. Discover testimonials and reviews from those who have experienced this Maldivian literary souvenir.',
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  res.setHeader('X-Page-Handler', 'reviews');
  ...
  try {
    ...
    const jsonLdScript = `<script type="application/ld+json">${JSON.stringify(webPageSchema)}</script>
<script type="application/ld+json">${JSON.stringify(breadcrumb)}</script>`;

    // Enhanced meta tags for AI search
    const enhancedMeta = `
    <meta name="keywords" content="Heal in Paradise reviews, book reviews, reader testimonials, Maldivian poetry reviews, Hawla Riza reviews" />
    ...
    console.error('Error in reviews page handler:', error);
